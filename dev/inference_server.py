...
def run_inference_endpoint():
    ...
    export_path = Path('strong_exports') / strong_export_file
...
def list_weak_models():
    ...
    for filepath in Path(".").glob("weak_model_*.json"):
        try:
...
def export_strong_traces():
    ...
    exports_dir = Path('strong_exports')
...
def fetch_traces():
    ...
    try:
        # Run get_traces.py script
        result = subprocess.run(
            ['python', 'get_traces.py', project_name],
            capture_output=True,
            text=True,
        ...
        if result.returncode != 0:
            return jsonify({
                'error': f'Failed to fetch traces: {result.stderr}'
            }), 500
        ...
        return jsonify({'error': 'Trace fetch timed out (>5 minutes)'}), 500
    except Exception as e:
